...
[Claude Code](https://claude.ai/code) 社区技能市场。浏览、安装和分享可复用的 Claude Code 技能。

## 什么是技能？

技能是一个封装了专业知识的可复用提示词，以 `SKILL.md` 文件形式存在。它教会 Claude 如何执行特定任务——从磁盘清理到 PDF 生成再到 API 集成。技能可以通过 `/技能名` 斜杠命令手动调用，也可以由 Claude 在合适的场景下自动触发。
...
- **文件路径**：`~/.claude/skills/<技能名>/SKILL.md`（macOS/Linux）
- **文件路径**：`%USERPROFILE%\.claude\skills\<技能名>\SKILL.md`（Windows）

以 `disk-cleanup` 为例，`SKILL.md` 内容地址：
...
3. 遵循上方的[技能格式](#技能格式)
4. 将技能添加到 [`registry.json`](./registry.json) 和本 README 的技能列表中
5. 提交 Pull Request
...
🤖 为 [Claude Code](https://claude.ai/code) 社区构建
